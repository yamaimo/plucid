...
int memsearch(MEMPTR* place, int n, STPPTR s, STPPTR t, STPPTR p)
{
    D(30) {
        bar(ddepth);
        ddepth++;
    ...
    int i = ((59 * (int)s) + (39 * (int)t) + (7 * (int)p) + n) % NHASHSIZE;
    if (i < 0)
    {
        i = -i;
    }
    ...
    else
    {
        ...
        {
            if (exprtable[n]-> f != F_INPUT
                && exprtable[n]->f != F_FILE
                && exprtable[n]->f != F_FILTER)
            {
                fprintf(
                    stderr,
                ...
                fprintf(
                    stderr,
                    "Variable '%s' is defined in terms of its own current or future value \n"
...
STPPTR ssearch(COORDS hd)
{
    int i = (39 * hd.d1 + 19 * hd.d2 + hd.d3) % SHASHSIZE;
    if (i < 0)
    {
        i = -i;
    }
...
STPPTR stpsearch(long hd, STPPTR tl, STPPTR table[])
{
    int i = (hd) % THASHSIZE;
    if (i < 0)
    {
