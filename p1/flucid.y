...
constant
    : CONST
        {
            $$.eptr = f_connode($1.numb);
        }
    | WORD
        {
            $$.eptr = connode("word", $1.resptr->strg);
        }
    | STRING_QUOTED
        {
            $$.eptr = connode("string", $1.resptr->strg);
        }
    ;

list
    : CLSTART listbody CLEND
        {
            $$.eptr = $2.eptr;
        }
    | CLSTART CLEND
        {
            $$.eptr = connode("special", "nil");
        }
    ;

clist
    : LSTART clistbody LEND
        {
            $$.eptr = $2.eptr;
        }
    | LSTART LEND
    ...
    : constant
        {
            $$.eptr = opnode(
                ...
                filenode(in_files[in_index].in_line, curr_index));
        }
    | clist
        {
            $$.eptr = opnode(
    ...
    | expr COMMA listbody
        {
            $$.eptr = opnode(
                ...
                filenode(in_files[in_index].in_line, $2.resptr->len));
        }
    ;
    ...
    | C2FUNC '(' expr COMMA expr ')'
        {
            $$.eptr = opnode(
                $1.resptr->strg,
                0,
                exprlist2($3.eptr, $5.eptr),
                filenode($1.resptr->line, $1.resptr->len));
        }
    | R0FUNC
        {
            $$.eptr = connode("special", $1.resptr->strg);
        }
    | R1FUNC expr
        {
            $$.eptr = opnode(
    ...
    | LU3FUNC '(' expr COMMA expr COMMA expr ')'
        {
            $$.eptr = opnode(
                $1.resptr->strg,
                3,
                exprlist3($3.eptr, $5.eptr, $7.eptr),
                filenode($1.resptr->line, $1.resptr->len));
        }
    ...
    : IF expr THEN expr ending
        {
            $$.eptr = opnode(
                "if",
                0,
                exprlist3($2.eptr, $4.eptr, $5.eptr),
                filenode($1.resptr->line, $1.resptr->len));
        }
            ...
            $$.eptr=opnode(
                ...
                exprlist3(
                    opnode(
                        "eq",
                        0,
                        exprlist2(expval[expvalcnt], $1.eptr),
                        filenode($2.resptr->line, $2.resptr->len)),
                    $3.eptr,
                    $5.eptr),
                filenode($2.resptr->line, $2.resptr->len));
...
condbody
    : expr COLON expr ';' condbody
        {
            $$.eptr = opnode(
                "if",
                0,
                exprlist3($1.eptr, $3.eptr, $5.eptr),
                filenode($2.resptr->line, $2.resptr->len));
        }
    | expr error
        {
...
identlist
    : IDENT
        {
            $$.eptr = identlistnode(NULL, $1.resptr->strg);
            idcount++;
        }
    | identlist COMMA IDENT
        {
            $$.eptr = identlistnode($1.eptr, $3.resptr->strg);
            idcount++;
        }
    | identlist error
        {
