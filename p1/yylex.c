...
int yylex(void)
{
    ...
    /*
     * NOTE: the followings are global variables
     * int c;
     * STRING s;
     * int startoflex;
     * int curr_index;
    ...
    if ((isalpha(c) || c=='@') && c != EOF)
    {
        ...
        else
        {
            ...
            yylval.resptr->line = in_files[in_index].in_line;
            yylval.resptr->len  = startoflex;
            return (const_list != 0) ? WORD : IDENT;
        }
    }
...
static void inclusion(void)
{
    ...
    {
        my_yyerror(
            "< or \" expected after an include statement\n",
            "cannot continue compilation");
    }
    if (c == '"')
    {
        for (s = buffer; (c = lexgetc()) != '"'; *s++ = c)
        {
            if (c == EOF)
            {
                my_yyerror(
                    "reached EOF and have not found \" to close string of include\n",
                    "cannot continue compilation");
                my_exit(1);
            }
        }
        ...
        if (isalpha(c))
        {
            while (isalpha(c) || isdigit(c) || c=='.')
            {
                *s = c;
                ...
                {
                    my_yyerror(
                        "reached EOF and have not found >",
                        "cannot continue compilation");
                    my_exit(1);
                }
            }
            *s = '\0';
            ...
            {
                my_yyerror(
                    "> needed to close include statement",
                    "cannot continue compilation");
                my_exit(1);
            }
            ...
            {
                my_yyerror(
                    "; expected after include <filename>",
                    "cannot continue compilation");
                my_exit(1);
            }
        }
...
STRING getstring(char c)
{
    ...
    while (c != '\'')
    {
        if (c == '\\')
        {
            ...
            {
                my_yyerror(
                    "EOF reached with no closing quote for string",
                    " ");
                my_exit(1);
            }
            switch (c)
        ...
        if (l == 255)
        {
            ...
            for (i = 0; i < sptr - 2; i++)
            {
                if (isprint(strstrg[i]))
                {
                    putc(' ', stderr);
                }
                else
                {
                    putc(strstrg[i], stderr);
                }
            }
            fprintf(stderr, "^\n");
        }
...
static float getnum(char c)
{
    ...
    for (c = lexgetc(); isdigit(c); c = lexgetc())
    {
        ...
        {
            my_yyerror(
                "too many digits in numeric constant. Max is 7",
                "\n");
            while (isdigit(c))
            {
                c = lexgetc();
