/*******************************************************

            Flucid Interpreter

             Copyright 1986
           All Rights Reserved

            Dr. Antony Faustini
          Arizona State University


********************************************************/

#include <ctype.h>
#include <stdio.h>
#include <string.h>

#include "cmanifs.h"
#include "cglobals.h"
#include "expr.h"
#include "ident.h"

STRING strsave(char* s);    /* main.c */
void my_exit(int n);        /* main.c */

int yylex(void);
int findword(STRING s);

static int lexgetc(void);
static STRING getword(char c);
static int is_sign(char c);
static STRING code_to_char(STRING s);
static STRING getstring(char c);
static float getnum(char c);
static int keyfind(STRING s);
static int iswhite(int c);

int yylex(void)
{
    while (iswhite(c = lexgetc())) ;

    if ((c == '@' || c == '_' || isalpha(c)) && c != EOF)
    {
        int k;
        s = getword(c);
        if ((k = keyfind(s)) != MAXFUNCS && cconst)
        {
            cconst = false_;
            if (ftable[k].type == WRD)
            {
                wordval = true_;
            }
            if (ftable[k].nargs == 0)
            {
                yylval.value = ftable[k].type;
            }
            else
            {
                yylval.value = k;
            }
            return ftable[k].type;
        }
        yylval.ident = handle_ident(s, l);
        return IDENT;
    }
    if (isdigit(c) || c=='~')
    {
        yylval.numb = getnum(c);
        return NUMB;
    }
    if (c == '`')
    {
        c = lexgetc();
        if (wordval)
        {
            yylval.value = findword(getstring(c));
            wordval = false_;
        }
        else
        {
            yylval.value = findword(getstring(c));
        }
        return STRING_QUOTED;
    }
    if (c == '[')
    {
        cconst = true_;
    }
    return c;
}

static int lexgetc(void)
{
    int c;

    if (peekc != 0)
    {
        c = peekc;
        peekc = 0;
        return c;
    }
    else if ((c = getc(lexin)) == EOF)
    {
        if (in_index != 0)
        {
            in_index--;
            lexin = in_files[in_index].in_fdes;
            c = getc(lexin);
        }
    }
    if (c == '\n')
    {
        in_files[in_index].in_line++;
    }
    if (c == '\032')
    {
        my_exit(1);
    }
    return c;
}

/*
// same as findword?
int findstring(STRING s)
{
    int i;

    for (i = 0; i < stringcount; i++)
    {
        if (!strcmp(s, stringtable[i]))
        {
            break;
        }
    }
    if (i == stringcount)
    {
        stringcount++;
        stringtable[i] = strsave(s);
    }
    return i;
}
*/

int findword(STRING s)
{
    int i;

    for (i = 0; i < wordcount; i++)
    {
        if (!strcmp(s, wordtable[i]))
        {
            break;
        }
    }
    if (i == wordcount)
    {
        wordcount++;
        wordtable[i] = strsave(s);
    }
    return i;
}

static STRING getword(char c)
{
    STRING p = buffer;
    l = 1;

    switch (c)
    {
    case ';':
    case ',':
    case '.':
    case ')':
    case '"':
        *p++ = c;
        l++;
        c = lexgetc();
        break;
    case '%':
        *p++ = c;
        l++;
        c = lexgetc();
        if (c == ']' || c == ')')
        {
            *p++ = c;
            l++;
            c = lexgetc();
            break;
        }
        fprintf(stderr, "ERROR1");
        my_exit(1);
    case '(':
        *p++ = c;
        l++;
        c = lexgetc();
        if (c == '%')
        {
            *p++ = c;
            l++;
            c = lexgetc();
        }
        peekc = c;
        break;
    case '[':
        *p++ = c;
        l++;
        c = lexgetc();
        if (c == '%')
        {
            *p++ = c;
            l++;
            c = lexgetc();
            break;
        }
        fprintf(stderr, "ERROR2");
        my_exit(1);
    case '+':
    case '-':
    case '/':
    case '<':
    case '=':
    case '>':
    case '*':
    case '$':
    case '&':
    case ':':
    case '^':
    case '#':
        *p++ = c;
        l++;
        while (is_sign(c = lexgetc()))
        {
            *p++ = c;
            l++;
        }
        break;
    default:
        if (isalpha(c) || c == '_' || c == '@')
        {
            *p++ = c;
            l++;
            c = lexgetc();
            while (isalpha(c) || isdigit(c) || c == '_')
            {
                *p++ = c;
                l++;
                c = lexgetc();
            }
        }
        else
        {
          fprintf(stderr, "ERROR3");
          my_exit(1);
        }
    }
    /* now at end of word */
    peekc = c;
    *p = '\0';
    return buffer;
}

static int is_sign(char c)
{
    switch (c)
    {
    case ':':
    case '^':
    case '+':
    case '-':
    case '/':
    case '<':
    case '=':
    case '>':
    case '*':
    case '$':
    case '&':
    case '#':
        return true_;
    default:
        return false_;
    }
}

static STRING code_to_char(STRING s)
{
    char newstring[200];
    int i = 0;
    int l = 0;
    char c = s[i];

    while (c != '\0')
    {
        if (c == '\\')
        {
            i++;
            c = s[i];
            switch (c)
            {
            case 'n':
                newstring[l] = '\n';
                l++;
                break;
            case 't':
                newstring[l] = '\t';
                l++;
                break;
            case 'f':
                newstring[l] = '\f';
                l++;
                break;
            case 'b':
                newstring[l] = '\b';
                l++;
                break;
            case 'r':
                newstring[l] = '\r';
                l++;
                break;
            case '\\':
                newstring[l] = '\\';
                l++;
                break;
            case '\'':
                newstring[l] = '\'';
                l++;
                break;
            default:
                if ('0' <= c && c <= '7')
                {
                    int sum = 0;
                    for (int j = 1; j <= 3 && '0' <= c && c <= '7'; j++)
                    {
                        i++;
                        sum = sum * 8 + c - '0';
                        c = s[i];
                    }
                    newstring[l] = sum;
                    l++;
                }
                newstring[l] = c;
                l++;
            }
        }
        else
        {
            newstring[l] = c;
            l++;
        }
        i++;
        c = s[i];
    }
    newstring[l] = '\0';
    strcpy(s, newstring);
    return s;
}

static STRING getstring(char c)
{
    STRING p = buffer;
    l = 0;
    while (c != '\'')
    {
        if (c == '\\')
        {
            c = lexgetc();
            if (c == EOF)
            {
                yyerror("EOF reached with no closing quote for string");
                my_exit(1);
            }
            switch (c)
            {
            default:
                *p++ = '\\';
                l++;
            case '\\':
            case '`':
                break;
            }
        }
        switch (c)
        {
        case '\t':
            *p++ = '\\';
            l++;
            *p++ = 't';
            l++;
            break;
        case '\f':
            *p++ = '\\';
            l++;
            *p++ = 'f';
            l++;
            break;
        case '\b':
            *p++ = '\\';
            l++;
            *p++ = 'b';
            l++;
            break;
        default:
            *p++ = c;
            l++;
        }
        if (l == 255)
        {
            fprintf(stderr, "WARNING long string\n");
        }

        c = lexgetc();
        if (c == EOF)
        {
            yyerror("EOF reached with no closing quote for string");
            my_exit(1);
        }
    }
    /* we are now at the end of the string */
    *p = '\0';
    return strsave(code_to_char(buffer));
}

static float getnum(char c)
{
    int sign;
    int mansum;
    float expsum;
    float expcount;

    if (c == '~')
    {
        sign = -1;
        c = lexgetc();
        if (!isdigit(c))
        {
            yyerror("~ must be followed by a digit");
        }
    }
    else
    {
        sign = 1;
    }
    mansum = c - '0';
    expsum = 0;
    expcount = 1;
    for (c = lexgetc(); isdigit(c); c = lexgetc())
    {
        mansum = mansum * 10 + (c - '0');
    }
    if (c == '.')
    {
        for (c = lexgetc(); isdigit(c); c = lexgetc())
        {
            expsum = expsum * 10 + (c - '0');
            expcount = expcount * 10;
        }
    }
    peekc = c;
    return sign * (mansum + expsum / expcount);
}

static int keyfind(STRING s)
{
    int i;
//    for (i = 0; i < MAXFUNCS && strcmp(s, ftable[i].name); i++) ; // maybe bug.
    for (i = 0; i < MAXFUNCS && strcmp(s, ftable[i].name) != 0; i++) ;
    return i;
}

static int iswhite(int c)
{
    return (c == ' ' || c == '\n' || c == '\t');
}
